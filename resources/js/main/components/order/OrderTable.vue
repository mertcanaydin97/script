					...
					<template #expandedRowRender="orderItemData">
						...
						>
							<template #bodyCell="{ column, record }">
								...
								<a-radio-group v-model:value="record.status">
								<a-radio-button value="0" v-bind:name="record.xid" @click="changeOrderItemStatus(record.xid, '0')">{{$t("product.single_waiting_shipment")}}</a-radio-button>
								<a-radio-button value="1" v-bind:name="record.xid" @click="changeOrderItemStatus(record.xid, '1')">{{$t("product.single_shipped")}}</a-radio-button>
							</a-radio-group>
							</template>
